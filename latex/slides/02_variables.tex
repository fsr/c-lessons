...
\begin{frame}[fragile]{Floating point numbers}
	\begin{itemize}
		...
		\item Type a decimal point instead of a comma! (Since C was originally developed in the USA)
	\end{itemize}\ \\
	\ \\
	Example:
	\begin{lstlisting}[numbers=none]
...
\begin{frame}[fragile]{Characters}
	\begin{itemize}
		...
		\item Indented to represent a single character
...
\end{lstlisting}
\end{frame}
\begin{frame}[fragile]{Overwriting identifiers}
...
\begin{frame}[fragile]{Hello again!}
	...
	\end{itemize}
\end{frame}
\begin{frame}[fragile]{ASCII number explorer}
	\begin{itemize}
		\item Write a program that asks to input a character and prints its ASCII number
		\begin{itemize}
