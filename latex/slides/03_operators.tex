...
\begin{frame}[fragile]{Let's calc}
	Each time you type in a value or a variable, it gets evaluated as an expression. You
	 can use operators to make new expressions out of existing ones.
...
i1 / i2;	/* result is 1 (42/23 = [1]*23 + 19)
			 * division with integral numbers  */
i1 + s;		/* int and short, result is int */
i1 / f;		/* int and float, result is float
			 * division with rational numbers */
\end{lstlisting}
\end{frame}
\begin{frame}[fragile]{Syntatic sugar}
	As you have seen, you can use any expression on the right side of the assignment
	 operator.\\
...
b /= 42;	/* b = b / 42; */
b %= 2;		/* b = b % 2; */
c++;		/* c = c + 1; //evaluates to c */
++c;		/* c = c + 1; //evaluates to c + 1 */
c--;		/* c = c - 1; //evaluates to c */
--c;		/* c = c - 1; //evaluates to c - 1 */
\end{lstlisting}
\end{frame}
...
\begin{frame}{The truth about expressions}
	...
	If a value or a variable equals 0, its corresponding truth value is \textit{false}.
	 Otherwise it's \textit{true}.\\
...
a = (b =@\,@= (c = d));	/* Wat? */
\end{lstlisting}\ \\
	\ \\\textit{c++} expressions are evaluated before the increment while \textit{++c}
	 increments first (the same applies on C$--$):
	\begin{lstlisting}[numbers=none]
int c = 42;
int a = c++;	/* a is 42 */
int b = ++c;	/* b is 43 */
...
\begin{frame}{Boolean arithmetic}
	...
	\begin{itemize}
		\item \&\& for AND (results in \textbf{1} if both operands are true, else
		 \textbf{0})
		\item $||$ for OR (results in \textbf{1} if at least one operator is true, else
		 \textbf{0})
	...
	\begin{center}
		! \ $>$ \ \&\& \ $>$ \ $||$\\	
	\end{center}
	The evaluation of AND \& OR is done lazily, from left to right. If the left expression
	is \textit{true} (with OR) / \textit{false} (with AND), the right expression will not
	 be evaluated.
\end{frame}
...
\begin{frame}{Calculator Light}
	\begin{itemize}
		\item Write a program that asks the user for two decimal numbers and prints their
		 sum, difference, product, integer quotient and remainder.
		\item \textbf{Experts}: Implement a division with rational numbers instead.
	\end{itemize}
\end{frame}
\begin{frame}{Geometry}
	\begin{itemize}
		\item Write a program that asks for a radius and calculates the area of the
		 corresponding circle. (Let's assume $\pi$ == 3.14)
		\item \textbf{Experts}: ask for an additional number. Print 1, if the area is
		 below that number, else print zero.
	\end{itemize}
\end{frame}
